...
@download_controller.route("/dashboard/<report_id>/download", methods=["GET"])
def download_scan(report_id):
    report_response = requests.get(API_HOST + f"/splitast/api/report/file/{report_id}")
    ...
    if report_response.status_code == 200:
        file = BytesIO(report_response.content)
        return send_file(file, download_name="report.json", mimetype="application/json", as_attachment=True)
